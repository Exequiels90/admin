...
def editar_producto(id_producto):
    ...
    if request.method == "POST":
        ...
        conn.execute("""
            UPDATE productos
            SET nombre = ?, categoria_id = ?, precio_compra = ?, precio_venta = ?, activo = ?
            WHERE id_producto = ?
        """, (nombre, categoria_id, precio_compra, precio_venta, activo, id_producto))
...
def nuevo_lote():
        ...
        for i, (nombre, marca, categoria_id, cantidad, pcompra, pventa, codigo_existente) in enumerate(
            ...
            if nombre and cantidad and pcompra and pventa:
                if codigo_existente:
                    ...
                    cursor.execute("""
                        INSERT INTO productos (codigo, nombre, marca, categoria_id, precio_compra, precio_venta)
                        VALUES (?, ?, ?, ?, ?, ?)
                    """, (codigo, nombre, marca, categoria_id or None, float(pcompra), float(pventa)))
                    id_producto = cursor.lastrowid
    ...
    categorias = conn.execute("SELECT * FROM categorias").fetchall()
    productos_existentes = conn.execute("""
        SELECT id_producto, codigo, p.nombre, marca, precio_venta, c.nombre as categoria
        FROM productos p
        LEFT JOIN categorias c ON p.categoria_id = c.id_categoria
        WHERE p.activo = 1 AND p.eliminado = 0
        ORDER BY p.nombre
...
def ver_lote(id_lote):
    ...
    # Obtener detalles del lote
    detalles = conn.execute("""
        SELECT ld.*, p.codigo, p.nombre AS producto_nombre, p.marca
        FROM lotes_detalles ld
        JOIN productos p ON ld.id_producto = p.id_producto
        WHERE ld.id_lote = ?
        ORDER BY p.nombre
...
def eliminar_lote(id_lote):
    ...
    # Eliminar detalles del lote (esto activará los triggers para revertir el stock)
    conn.execute("DELETE FROM lotes_detalles WHERE id_lote = ?", (id_lote,))
...
def nueva_venta():
    ...
    clientes = conn.execute("SELECT id_cliente, nombre FROM clientes ORDER BY nombre").fetchall()
    productos = conn.execute("""
        SELECT p.id_producto, p.codigo, p.nombre, p.precio_venta, p.stock, c.nombre AS categoria
        FROM productos p
        LEFT JOIN categorias c ON p.categoria_id = c.id_categoria
        WHERE p.activo = 1 AND p.eliminado = 0 AND p.stock > 0
        ORDER BY p.nombre
...
def eliminar_venta(id_venta):
    ...
    # Eliminar detalles de la venta (esto activará los triggers para revertir el stock)
    conn.execute("DELETE FROM ventas_detalles WHERE id_venta = ?", (id_venta,))
...
def generar_codigos():
    conn = get_db_connection()
    productos = conn.execute("""
        SELECT p.id_producto, p.codigo, p.nombre, p.precio_venta, c.nombre AS categoria
        FROM productos p
        LEFT JOIN categorias c ON p.categoria_id = c.id_categoria
        WHERE p.activo = 1 AND p.eliminado = 0
        ORDER BY p.nombre
    """).fetchall()
    conn.close()
    return render_template("generar_codigos.html", productos=productos)
    ...
    try:
        ...
        for item in carrito:
            for i in range(item['cantidad']):
                ...
                # Nombre del producto (truncado si es muy largo) - PARTE SUPERIOR
                nombre = item['nombre']
                if len(nombre) > 25:
                    nombre = nombre[:22] + "..."
                p.drawString(x + 0.1 * cm, y + label_height - 0.5 * cm, nombre)
...
def dashboard():
    ...
    stock_bajo = conn.execute("SELECT COUNT(*) FROM productos WHERE stock < 10 AND eliminado = 0").fetchone()[0]
...
def reporte_productos():
    ...
    cursor.execute("""
        SELECT 
            ...
            ROUND(CAST(SUM(COALESCE(vd.cantidad, 0)) AS FLOAT) / AVG(p.stock) * 100, 2) as rotacion_categoria
        FROM categorias cat
        LEFT JOIN productos p ON cat.id_categoria = p.id_categoria AND p.eliminado = 0
        LEFT JOIN ventas_detalles vd ON p.id_producto = vd.id_producto
        LEFT JOIN ventas v ON vd.id_venta = v.id_venta AND v.eliminado = 0
        GROUP BY cat.id_categoria
        ORDER BY rotacion_categoria DESC
